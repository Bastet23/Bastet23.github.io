...
# --- VISUAL SETTINGS ---
# Changed to be barely noticeable, thin lines
@export var web_color: Color = Color(1.0, 1.0, 1.0, 0.15)
@export var bone_color: Color = Color(1.0, 1.0, 1.0, 0.15)
# Green shape is now completely solid (alpha 1.0)
@export var stat_fill_color: Color = Color(0.35, 0.7, 0.35, 1.0)
@export var stat_outline_color: Color = Color(0.8, 1.0, 0.8, 1.0) 
@export var req_web_color: Color = Color(1.0, 0.2, 0.2, 0.843)
# --- NEW: Mini Hexagon Background Color ---
@export var mini_bg_color: Color = Color("#96895c")
...
func update_labels():
	if not visual_root or not show_labels: return
...
func play_outcome_animation(is_success: bool, was_rerolled: bool = false):
	...
	if was_rerolled:
		...
		tween.tween_callback(func():
			var body = outcome_dot.get_node("SpiderBody")
...
func _get_radius_at_angle(stats_array: Array, angle_rad: float) -> float:
	...
	var max_pixel_radius = (min(size.x, size.y) / 2) - padding
	return linear_r * max_pixel_radius

# ==============================================================================
# INNER CLASS: HexagonDrawer 
# ==============================================================================
class HexagonDrawer extends Node2D:
	...
	func _draw():
		...
		var radius = (min(main_script.size.x, main_script.size.y) / 2) - main_script.padding
		if radius <= 0: return

		# --- 1. DRAW BACKGROUND & OUTER BORDER HEXAGON ---
		var outer_points = get_local_hex_points(center, radius)
		
		# Fill the background
		if not main_script.show_labels:
			draw_colored_polygon(outer_points, main_script.mini_bg_color)
			
		outer_points.append(outer_points[0]) # Close the loop
		
		# NEW: Added 'true' at the end for Anti-Aliasing!
		draw_polyline(outer_points, main_script.web_color, 2.0, true) 

		# --- 2. ONLY DRAW INNER WEB, SPOKES, AND REQS FOR THE LARGE HEXAGON ---
		if main_script.show_labels:
			# Inner rings 
			for i in range(1, 5):
				var r = (radius / 5) * i
				var points = get_local_hex_points(center, r) 
				points.append(points[0]) 
				draw_polyline(points, main_script.web_color, 1.0, true) # Anti-aliased

			# Spokes 
			var corners = get_local_hex_points(center, radius)
			for point in corners:
				draw_line(center, point, main_script.bone_color, 1.0, true) # Anti-aliased
			...
			if not main_script.current_reqs.is_empty():
				...
				draw_stat_shape(center, scaled_reqs, radius, main_script.req_web_color, true)

		# --- 3. ALWAYS DRAW THE GREEN STAT SHAPE ---
		var scaled_stats = []
		for val in main_script.current_stats:
			var normalized = clampf(val / main_script.max_stat_value, 0.0, 1.0)
			scaled_stats.append(normalized)
		
		draw_stat_shape(center, scaled_stats, radius, main_script.stat_fill_color, false)
	...
	func draw_stat_shape(center, stats_normalized, max_radius, color, is_req: bool):
		...
		if is_req:
			# Just draw a thin outline for requirements, anti-aliased
			draw_polyline(points, color, 1.0, true)
		else:
			# Draw the solid polygon
			draw_colored_polygon(points, color)
			# Draw a crisp outline over it, anti-aliased
			draw_polyline(points, main_script.stat_outline_color, 1.0, true)
