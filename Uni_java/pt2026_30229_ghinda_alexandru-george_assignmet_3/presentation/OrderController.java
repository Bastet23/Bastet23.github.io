...
public class OrderController {
    ...
    private void loadDropdownData() {
        ...
        for (Client c : clients) {
            view.getClientComboBox().addItem(new ComboItem(c.getId(), "ID: " + c.getId() + " | " + c.getName() + " (" + c.getAddress() + ")"));
        }

        List<Product> products = productBLL.findAllProducts();
        for (Product p : products) {
            view.getProductComboBox().addItem(new ComboItem(p.getId(), "ID: " + p.getId() + " | " + p.getName() + " - $" + p.getPrice() + " (Stock: " + p.getStock() + ")"));
        }
    }
