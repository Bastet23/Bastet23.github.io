...
import posixpath, argparse, fnmatch, platform
...
class Tester(threading.Thread):
    ...
    def perform(self):
        ...
        if verdict:
            print(Color.highlight(ColorEscape.BRIGHT_GREEN, "OK") + (" (with memory leaks)" if self.leak else ""))
            return 1
        else:
            print(Color.highlight(ColorEscape.BRIGHT_RED, "FAIL") + (" (with memory leaks)" if self.leak else ""))
            if VERBOSE:
                print("\tCommand: %s" % " ".join(self.cmd))
...
def main():
    ...
    parser.add_argument("-g", "--valgrind", 
        action = "store_true",
        help = "Evaluates using valgrind.")
    ...
    if args.docker or args.docker_persist:
        ...
        containerArgs = []
        if args.valgrind:
            containerArgs.append("-g")
        if args.verbose:
            containerArgs.append("-v")
        if args.test:
            containerArgs += ["-t", args.test]
        dh = DockerHelper()
        dh.runContainer()
        dh.copyDir(".")
        ...
        if compileRes == 0:
            ...
            tests = loadTests()
            if args.test:
                tests = [t for t in tests if fnmatch.fnmatch(t[0], args.test)]
                if len(tests) == 0:
            ...
            if compileRes == 1:
                ...
                score = score * 0.9
            if Tester.leaks:
                print(Color.highlight(ColorEscape.RED, "There were some memory leaks. A 10% penalty will be applied."))
                score = score * 0.9
